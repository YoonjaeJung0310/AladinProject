...
void APlayableAladin::Tick(float DeltaTime)//매 프레임마다 호출
{
	...
	if (GEngine)
	{
		// 화면 좌측 상단에 현재 캐릭터의 속력(Speed)을 매 프레임 갱신하여 출력
		GEngine->AddOnScreenDebugMessage(
			-1,            // 메시지 키 (-1: 기존 메시지와 겹치지 않고 계속 추가)
			0.0f,          // 유지 시간 (0.0f: 1프레임 동안만 유지되어 실시간 갱신에 적합)
			FColor::White, // 텍스트 색상
			FString::Printf(TEXT("Speed: %.1f"), GetVelocity().Size()) // 3차원 속도 벡터의 크기(속력) 출력
		);
	}
}
